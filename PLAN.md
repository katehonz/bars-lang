...
### Задачи
- [ ] **10.1** Higher-order функции: `map`, `filter`, `reduce`, `for-each` (върху вектори)
- [ ] **10.2** String ops: `split`, `join`, `trim`, `substring`, `concat`
- [ ] **10.3** Math: `pow`, `sqrt`, `sin`, `cos` (C math library wrappers)
- [ ] **10.4** Error handling: `Result` и `Option` ADTs + helper macros (`try!`, `when-let`)
- [ ] **10.5** Basic I/O: `read-line`, `slurp` (цял файл в string), `spit` (string в файл)
- [ ] **10.6** Testing helpers: `assert`, `assert=`
