...
;; Source of truth (hand-tuned 17.7–17.9). Validate: python3 tools/gen_types.py --check
        ...
        (if (str-eq? name "push") true
          (if (str-eq? name "pop") true
            (if (str-eq? name "map-get") true
              (if (str-eq? name "map-set") true
                (if (str-eq? name "conj") true
                  (if (str-eq? name "v-assoc") true
                    (if (str-eq? name "v-pop") true
                      (if (str-eq? name "vector-clone") true
                        (if (str-eq? name "map-clone") true
                          (str-eq? name "map-assoc"))))))))))))))
...
(defn builtin_env [ctx]
  (let [arith (mono_scheme (T_Fun (i64_i64) (T_I64)))
    ...
    (do (env_insert env "+" arith)
        ...
        (env_insert env "map-clone" unary)
        (env_insert env "map-assoc" (mono_scheme (T_Fun (i64_i64-i64) (T_I64))))
        (env_insert env "vector-clone" unary)
        (env_insert env "conj" bin)
        (env_insert env "v-assoc" (mono_scheme (T_Fun (i64_i64-i64) (T_I64))))
        (env_insert env "v-pop" unary)
