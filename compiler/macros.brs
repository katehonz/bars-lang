  ...
  (if (str-eq? name "when")
    ...
    (if (str-eq? name "unless")
      ...
      (if (str-eq? name "when-not")
        (expand-unless expr macs)
        (if (str-eq? name "if-not")
          (expand-if-not expr macs)
              ...
              (if (str-eq? name "partial")
                ...
                (if (str-eq? name "complement")
                  ...
                  (if (str-eq? name "constantly")
                    ...
                    (if (str-eq? name "juxt")
                      (expand-juxt expr macs)
                      (if (str-eq? name "comp")
                        (expand-comp expr macs)
                        (if (str-eq? name "fnil")
                          (expand-fnil expr macs)
                          (if (str-eq? name "not-any?")
                            (expand-not-any expr macs)
                            (if (str-eq? name "not-every?")
                              (expand-not-every expr macs)
...
(defn expand-if-not [expr macs]
  (let [n (count expr)]
    (if (< n 3)
      (mk-nil)
      (let [not-call (vector)
            _ (push not-call (mk-sym "not"))
            _ (push not-call (expand-expr (get expr 1) macs))
            then-e (expand-expr (get expr 2) macs)
            else-e (if (>= n 4) (expand-expr (get expr 3) macs) (mk-nil))]
        (mk-if not-call then-e else-e)))))
...
;; (complement f) => (fn [__cx] (not (f __cx)))
(defn expand-complement [expr macs]
  (let [n (count expr)]
    (if (< n 2)
      (mk-nil)
      (let [f (expand-expr (get expr 1) macs)
            x (mk-sym "__cx")
            call (vector)
            _ (push call f)
            _ (push call x)
            notc (vector)
            _ (push notc (mk-sym "not"))
            _ (push notc call)
            out (vector)]
        (do (push out (mk-special 16 "fn"))
            (push out (wrap-vec (vector x)))
...
(defn expand-juxt [expr macs]
  (let [n (count expr)
        x (mk-sym "__jx")
        vec-call (vector)
        _ (push vec-call (mk-sym "vector"))
        _ (loop [i 1]
            (if (>= i n) 0
...
;; (fnil f d) => (fn [x] (f (if x x d)))
;; (fnil f d1 d2) / (fnil f d1 d2 d3) — same for 2–3 args (0/nil → default)
(defn expand-fnil [expr macs]
  (let [n (count expr)]
    (if (< n 3)
      (mk-nil)
      (if (> n 5)
        ...
        (let [f (expand-expr (get expr 1) macs)
              ...
              _ (loop [i 0]
                  (if (>= i nd) 0
                    (let [p (mk-sym (str-concat "__fn" (str-from-i64 i)))
                          d (expand-expr (get expr (+ i 2)) macs)
                          arg (mk-if p p d)]
                      (do (push params p)
                          (push call arg)
                          (recur (+ i 1))))))
              out (vector)]
          (do (push out (mk-special 16 "fn"))
              (push out (wrap-vec params))
              (push out call)
              out))))))

;; (not-any? pred coll) => (if (some pred coll) 0 1)
(defn expand-not-any [expr macs]
  (let [n (count expr)]
    (if (< n 3)
      (mk-num 1)
      (mk-if (mk-call "some" (vector
               (expand-expr (get expr 1) macs)
               (expand-expr (get expr 2) macs)))
             (mk-num 0)
             (mk-num 1)))))

;; (not-every? pred coll) => (if (every? pred coll) 0 1)
(defn expand-not-every [expr macs]
  (let [n (count expr)]
    (if (< n 3)
      (mk-num 0)
      (mk-if (mk-call "every?" (vector
               (expand-expr (get expr 1) macs)
               (expand-expr (get expr 2) macs)))
             (mk-num 0)
             (mk-num 1)))))
