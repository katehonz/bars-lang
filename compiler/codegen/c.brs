...
;; True when any HIR line references the TLS runtime fns.
...
(defn c-header [with-tls]
  (let [lines (vector)]
    (do (lines-push lines "/* Generated by Bars C backend */")
        ...
        ;; TLS client fns live in runtime/bars_tls.o (linked on demand)
        (if with-tls
          (do (lines-push lines "int64_t bars_tls_connect(int64_t, int64_t, int64_t);")
              ...
              (lines-push lines "int64_t bars_tls_listen(int64_t, int64_t, int64_t);")
              (lines-push lines "int64_t bars_tls_accept(int64_t);")
              (lines-push lines "int64_t bars_tls_server_close(int64_t);"))
          0)
        (lines-push lines "")
        lines)))
      ...
      (if (>= i n)
        (let [flushed (if (= in-func 1)
                        (do (lines-push body "}")
                            ...
                            all)
                        all)
              out (c-header (hir-mentions-tls? hir-lines))
              wrap (c-main-wrapper)]
