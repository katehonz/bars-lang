...
impl LanguageServer for Backend {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        // Built-in specials + macros (Phase 17 control/HOF)
        let builtins = [
            ("def", "Global binding: (def name value)"),
            ("defn", "Define a function: (defn name [params] body)"),
            ("defmacro", "User macro: (defmacro name [params] body)"),
            ("defconst", "Constant as zero-arg fn: (defconst Name value)"),
            ("let", "Local bindings: (let [x 1 y 2] body)"),
            ("if", "Conditional: (if cond then else)"),
            ("if-let", "Bind when truthy: (if-let [x e] then else)"),
            ("if-not", "Negated if: (if-not c then else)"),
            ("when", "If without else: (when c body…)"),
            ("when-let", "When + bind: (when-let [x e] body…)"),
            ("when-not", "Alias of unless"),
            ("unless", "If not: (unless c body…)"),
            ("cond", "Multi-branch: (cond c1 e1 c2 e2 :else e)"),
            ("case", "Equality dispatch: (case e c1 r1 default?)"),
            ("do", "Sequence expressions: (do expr1 expr2 ...)"),
            ("loop", "Tail-recursive loop: (loop [bindings] body)"),
            ("recur", "Recurse in loop: (recur args...)"),
            ("fn", "Anonymous function / closure: (fn [params] body)"),
            ("match", "Pattern match: (match expr [pattern body] ...)"),
            ("doseq", "Iterate for side effects: (doseq [x coll] body…)"),
            ("for", "List comprehension → vector: (for [x coll] body)"),
            ("dotimes", "Index loop: (dotimes [i n] body…)"),
            ("while", "While loop: (while cond body…)"),
            ("and", "Short-circuit and (macro)"),
            ("or", "Short-circuit or (macro)"),
            ("partial", "Partial application: (partial f a…)"),
            ("comp", "Compose: (comp f g h) rightmost first"),
            ("juxt", "Apply several fns: (juxt f g)"),
            ("complement", "Negate predicate: (complement f)"),
            ("constantly", "Constant fn: (constantly v)"),
            ("fnil", "Nil/0 defaults: (fnil f d…)"),
            ("apply", "Call with arg vector: (apply f args)"),
            ("some", "First truthy (pred x)"),
            ("every?", "All truthy (pred x)"),
            ("map", "HOF map / empty hash-map constructor"),
            ("filter", "Filter by predicate"),
            ("reduce", "Left fold"),
            ("deftype", "Algebraic data type: (deftype Name [Ctor fields...])"),
            ("defstruct", "Struct definition: (defstruct Name fields...)"),
            ("deftrait", "Trait: (deftrait T [m…] (default m …)*)"),
            ("impl", "Implement trait: (impl T for Type …)"),
            ("extern", "C FFI declaration"),
            ("require", "Module: (require \"path\" :as alias)"),
